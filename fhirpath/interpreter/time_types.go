...
func NewTimeHMSNWithPrecision(hour int, minute int, second int, nanosecond int, precision DateTimePrecisions) TimeAccessor {
	return NewTimeHMSNWithPrecisionAndSource(hour, minute, second, nanosecond, precision, nil)
}

func NewTimeHMSNWithPrecisionAndSource(hour int, minute int, second int, nanosecond int, precision DateTimePrecisions, source interface{}) TimeAccessor {
	if precision < HourTimePrecision || precision > NanoTimePrecision {
		log.Fatal("Invalid time precision %d", precision)
	}
...
func timeValueEqual(t1 TimeAccessor, t2 TimeAccessor) bool {
	return TemporalPrecisionEqual(t1, t2) &&
		t1.Hour() == t2.Hour() &&
		t1.Minute() == t2.Minute() &&
		t1.Second() == t2.Second() &&
...
func (t *timeType) Add(quantity QuantityAccessor) (TemporalAccessor, error) {
	...
	if precision < HourTimePrecision {
		return nil, fmt.Errorf("Quantity precision not allowed for time type: %s", quantity.String())
	}
	...
	if res.Day() != 1 || res.Month() != 1 || res.Year() != 1 {
		return nil, fmt.Errorf("Resulting time is not valid: %s", res.String())
	}
