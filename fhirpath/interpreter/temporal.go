...
func addQuantityTemporalDuration(temporal DateTemporalAccessor, quantityValue NumberAccessor, quantityPrecision DateTimePrecisions) (time.Time, error) {
	return addQuantityDateTimeDuration(temporal.Time(), temporal.Precision(), quantityValue, quantityPrecision)
}

func addQuantityDateTimeDuration(t time.Time, precision DateTimePrecisions, quantityValue NumberAccessor, quantityPrecision DateTimePrecisions) (time.Time, error) {
	if precision < quantityPrecision {
		...
		default:
			log.Fatalf("Invalid date/time precision: %d", precision)
		}
	...
	case HourTimePrecision:
		t = t.Add(time.Duration(quantityValue.Int()) * time.Hour)
	case MinuteTimePrecision:
		t = t.Add(time.Duration(quantityValue.Int()) * time.Minute)
	case SecondTimePrecision:
		t = t.Add(time.Duration(quantityValue.Int()) * time.Second)
	case NanoTimePrecision:
		t = t.Add(time.Duration(quantityValue.Int()) * time.Nanosecond)
	default:
		log.Fatalf("Invalid date/time precision: %d", quantityPrecision)
	}
...
func quantityDateTimePrecision(q QuantityAccessor) (NumberAccessor, DateTimePrecisions, error) {
	...
	if qu == nil {
		return nil, NanoTimePrecision, fmt.Errorf("Quantity has no valid Date/Time unit: %s", unit.String())
	}
